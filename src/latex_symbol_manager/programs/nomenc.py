...
def print_nomenclature(symbols, stream, skip_empty=True):
    ...
    for symbol in list(symbols.values()):
        ...
        if "nomenc-exclude" in symbol.other:
            warn(
                "Skipping symbol %s because of " "nomenc-exclude" % symbol.symbol, False
            )
            continue

        if symbol.nomenclature is None:
            if skip_empty:
                warn("Skipping symbol %s because of skip_empty." % symbol.symbol)
                continue

            if symbol.nargs != 0:
                warn("Skipping symbol %s because it has args." % symbol.symbol, False)
                continue
